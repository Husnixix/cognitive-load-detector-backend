...
from app.repositary.cognitive_load_respositary import CognitiveStateRespositary

class SQLCognitiveStateRepository(CognitiveStateRespositary):
    def __init__(self):
        self.collection = ConfigureDatabase().get_cognitive_states_collection("cognitive_states")

    def save(self, cognitive_state: CognitiveState):
        document = {
        ...
        cognitive_state.id = result.inserted_id
    ...
    def get_cognitive_state_history(self) -> list[CognitiveState]:
        ...
        return [
            CognitiveState(
                id=str(documents["_id"]),
                start_time=documents["start_time"],
                end_time=documents["end_time"],
                facial_cue_data=documents["facial_cue_data"],
                keystroke_data=documents["keystroke_data"],
                cognitive_state_data=documents["cognitive_state_data"],
            )
